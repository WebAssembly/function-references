...
struct
  ...
  let init p dt = Lib.Promise.fulfill p (Def dt)
  let alloc dt = let p = alloc_uninit () in init p dt; p

  let def_of x =
    match Lib.Promise.value x with
    | Def dt -> dt
    | _ -> assert false
...
let sem_module_type (Syn.ModuleType (dts, its, ets)) =
  let c = List.map (fun _ -> Sem.alloc_uninit ()) dts in
  List.iter2 (fun x dt -> Sem.init x (sem_def_type c dt)) c dts;
